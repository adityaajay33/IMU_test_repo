...
// According to the data sheet PS or Pin (#07) should be grounded for SPI and set to VDDIO for I2C.
		...
		HAL_GPIO_WritePin(GPIO_PIN_2, GPIO_PIN_RESET);


		//SPI command to read data from specific register
		status = HAL_Master_TransmitReceive(&hspi1, TRANSMIT_MESSAGE, RECEIVE_MESSAGE, MESSAGE_LEN, TIMEOUT_DELAY);


		//Error to console if status is not OK
		if(status!=HAL_OK){
			HAL_UART_Transmit(&huart2, (uint8_t *)uart_buf_fail, uart_buf_fail_len, 100);
		}
		...
		uint16_t raw_temp = ((RECEIVE_MESSAGE[1] << 8) | (RECEIVE_MESSAGE[2]/32));
		...
		HAL_UART_Transmit(&huart2, (uint8_t *)uart_result_temp, uart_result_temp_len, 100);
